...
#' ##         ...)
#' 
#' @param x an input object on which to assess balance. Can be the output of a call to a balancing function in another package or a formula or data frame. Input to this argument will determine which \code{bal.tab()} method is used. Each input type has its own documentation page, which is linked in the See Also section below. Some input types require or allow additional arguments to be specified. For inputs with no dedicated method, the \link[=bal.tab.default]{default method} will be dispatched. See Details below.
...
#' @param int \code{logical} or \code{numeric}; whether or not to include 2-way interactions of covariates included in \code{covs} and in \code{addl}. If \code{numeric}, will be passed to \code{poly} as well.
#' @param poly \code{numeric}; the highest polynomial of each continuous covariate to display. For example, if 2, squares of each continuous covariate will be displayed (in addition to the covariate itself); if 3, squares and cubes of each continuous covariate will be displayed, etc. If 1, the default, only the base covariate will be displayed. If \code{int} is numeric, \code{poly} will take on the value of \code{int}.
...
#' @param continuous whether mean differences for continuous variables should be standardized (\code{"std"}) or raw (\code{"raw"}). Default \code{"std"}. Abbreviations allowed. This option can be set globally using [set.cobalt.options()].
#' @param binary whether mean differences for binary variables (i.e., difference in proportion) should be standardized (\code{"std"}) or raw (\code{"raw"}). Default \code{"raw"}. Abbreviations allowed. This option can be set globally using [set.cobalt.options()].
#' @param s.d.denom \code{character}; how the denominator for standardized mean differences should be calculated, if requested. See [col_w_smd()] for allowable options. If weights are supplied, each set of weights should have a corresponding entry to \code{s.d.denom}. Abbreviations allowed. If left blank and weights, subclasses, or matching strata are supplied, \code{bal.tab()} will figure out which one is best based on the \code{estimand}, if given (for ATT, \code{"treated"}; for ATC, \code{"control"}; otherwise \code{"pooled"}) and other clues if not.
#' @param thresholds a named vector of balance thresholds, where the name corresponds to the statistic (i.e., in \code{stats}) that the threshold applies to. For example, to request thresholds on mean differences and variance ratios, one can set \code{thresholds = c(m = .05, v = 2)}. Requesting a threshold automatically requests the display of that statistic. When specified, extra columns are inserted into the Balance table describing whether the requested balance statistics exceeded the threshold or not. Summary tables tallying the number of variables that exceeded and were within the threshold and displaying the variables with the greatest imbalance on that balance measure are added to the output.
#' @param weights a vector, list, or \code{data.frame} containing weights for each unit, or a string containing the names of the weights variables in \code{data}, or an object with a [get.w()] method or a list thereof. The weights can be, e.g., inverse probability weights or matching weights resulting from a matching algorithm.
#' @param cluster either a vector containing cluster membership for each unit or a string containing the name of the cluster membership variable in \code{data} or the input object. See \code{\link[=class-bal.tab.cluster]{bal.tab.cluster}} for details.
#' @param imp either a vector containing imputation indices for each unit or a string containing the name of the imputation index variable in \code{data} or the input object. See \code{\link[=class-bal.tab.imp]{bal.tab.imp}} for details. Not necessary if \code{data} is a \code{mids} object.
#' @param pairwise whether balance should be computed for pairs of treatments or for each treatment against all groups combined. See \code{\link[=class-bal.tab.multi]{bal.tab.multi}} for details. This can also be used with a binary treatment to assess balance with respect to the full sample.
...
#' @param abs \code{logical}; whether displayed balance statistics should be in absolute value or not. 
#' @param subset a \code{logical} or \code{numeric} vector denoting whether each observation should be included or which observations should be included. If \code{logical}, it should have length equal to the number of units. \code{NA}s will be treated as \code{FALSE}. This can be used as an alternative to \code{cluster} to examine balance on subsets of the data.
#' @param quick \code{logical}; if \code{TRUE}, will not compute any values that will not be displayed. Set to \code{FALSE} if computed values not displayed will be used later.
#' @param ... for some input types, other arguments that are required or allowed. Otherwise, further arguments to control display of output. See \link[=display-options]{display options} for details.
#' 
#' @return
#' An object of class \code{"bal.tab"}. The use of continuous treatments, subclasses, clusters, and/or imputations will also cause the object to inherit other classes. The class \code{"bal.tab"} has its own \code{print()} method ([print.bal.tab()]), which formats the output nicely and in accordance with print-related options given in the call to \code{bal.tab()}, and which can be called with its own options.
#' 
...
#' 
#' @seealso 
#' For information on the use of \code{bal.tab()} with specific types of objects, use the following links:
#' \itemize{
#'     \item{[bal.tab.matchit()] for the method for objects returned by \pkg{MatchIt}.
#'     }
#'     \item{[bal.tab.weightit()] for the method for \code{weightit} and \code{weightitMSM} objects returned by \pkg{WeightIt}.
#'     }
#'     \item{[bal.tab.ps()] for the method for \code{ps}, \code{mnps}, and \code{iptw} objects returned by \pkg{twang} and for \code{ps.cont} objects returned by \pkg{twangContinuous}.
#'     }
#'     \item{[bal.tab.Match()] for the method for objects returned by \pkg{Matching}.
#'     }
#'     \item{[bal.tab.optmatch()] for the method for objects returned by \pkg{optmatch}.
#'     }
#'     \item{[bal.tab.cem.match()] for the method for objects returned by \pkg{cem}.
#'     }
#'     \item{[bal.tab.CBPS()] for the method for objects returned by \pkg{CBPS}.
#'     }
#'     \item{[bal.tab.ebalance()] for the method for objects returned by \pkg{ebal}.
#'     }
#'     \item{[bal.tab.designmatch()] for the method for objects returned by \pkg{designmatch}.
#'     }
#'     \item{[bal.tab.mimids()] for the method for objects returned by \pkg{MatchThem}.
#'     }
#'     \item{[bal.tab.sbwcau()] for the method for objects returned by \pkg{sbw}.
#'     }
#'     \item{[bal.tab.formula()] and [bal.tab.data.frame()] for the methods for \code{formula} and data frame interfaces when the user has covariate values and weights (including matching weights) or subclasses or wants to evaluate balance on an unconditioned data set. For data that corresponds to a longitudinal treatment (i.e., to be analyzed with a marginal structural model), see [bal.tab.time.list()].
#'     }
#' }
#' 
#' @examples 
