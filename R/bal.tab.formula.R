...
#' 
#' @description Generates balance statistics for unadjusted, matched, weighted, or stratified data using either a \code{data.frame} or formula interface.
#' 
#' @inheritParams bal.tab
#' @param x either a \code{data.frame} containing covariate values for each unit or a \code{formula} with the treatment variable as the response and the covariates for which balance is to be assessed as the terms. If a formula is supplied, all terms must be present as variable names in \code{data} or the global environment.
#' @param treat either a vector containing treatment status values for each unit or a string containing the name of the treatment variable in \code{data}. Required for the \code{data.frame} method.
#' @param s.d.denom \code{character}; how the denominator for standardized mean differences should be calculated, if requested. See [col_w_smd()] for allowable options. Abbreviations allowed. If weights are supplied, each set of weights should have a corresponding entry to \code{s.d.denom}; a single entry will be recycled to all sets of weights. If left blank and one of \code{weights}, \code{subclass}, or \code{match.strata} are supplied, \code{bal.tab()} will figure out which one is best based on \code{estimand}, if given (for ATT, `"treated"`; for ATC, `"control"`; otherwise "pooled") and other clues if not.
#' @param subclass optional; either a vector containing subclass membership for each unit or a string containing the name of the subclass variable in \code{data}.
#' @param match.strata optional; either a vector containing matching stratum membership for each unit or a string containing the name of the matching stratum variable in \code{data}. See Details.
#' @param method \code{character}; the method of adjustment, if any. If \code{weights} are specified, the user can specify either "matching" or "weighting"; "weighting" is the default. If multiple sets of weights are used, each must have a corresponding value for \code{method}, but if they are all of the same type, only one value is required. If \code{subclass} is specified, "subclassification" is the default. Abbreviations allowed. The only distinction between "matching" and "weighting" is how sample sizes are displayed.
#' @param estimand \code{character}; whether the desired estimand is the "ATT", "ATC", or "ATE" for each set of weights. This argument can be used in place of \code{s.d.denom} to specify how standardized differences are calculated.
#' @param focal the name of the focal treatment when multi-category treatments are used. See \code{\link[=class-bal.tab.multi]{bal.tab.multi}} for details.
#' 
#' @return 
