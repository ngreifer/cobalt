#' @title Balance Statistics for \code{MatchThem} Objects
#' 
#' @description
#' Generates balance statistics for \code{mimids} and \code{wimids} objects from \pkg{MatchThem}.
#' 
#' @inheritParams bal.tab
#' @param x a \code{mimids} or \code{wimids} object; the output of a call to \pkgfun{MatchThem}{matchthem} or \pkgfun{MatchThem}{weightthem}.
#' @param distance an optional formula or data frame containing distance values (e.g., propensity scores) or a character vector containing their names. If a formula or variable names are specified, \code{bal.tab()} will look in the argument to \code{data}, if specified. The distance measure generated by \code{matchthem()} or \code{weightthem()} is automatically included and named "distance" or "prop.score", respectively.
