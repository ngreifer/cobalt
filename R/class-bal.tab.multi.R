...
#'         
#' * `Pair.Balance`:For each pair of treatment groups, a regular \code{bal.tab} object containing a balance table, a sample size summary, and other balance assessment tools, depending on which options are specified. If \code{pairwise} is \code{FALSE}, the comparisons will be between each group and the groups combined (labeled "All") when \code{focal} is \code{NULL} and between each non-focal group and the focal group otherwise.
...
#' 
#' @note
#' In versions 4.3.1 and earlier, setting \code{pairwise = FALSE} would compare each group to the full adjusted sample. Now, each group is compared to the full \emph{un}adjusted sample (unadjusted except for \code{s.weights}, if supplied).
#'     
#' In versions 4.3.1 and earlier, \code{pairwise} was ignored with non-\code{NULL} \code{focal} and was automatically set to \code{FALSE}. \code{pairwise} can be specified and its default is now \code{TRUE}, so balance between all treatment groups will be computed by default rather than only between each non-group and the focal group. To recover previous functionality, set \code{pairwise = FALSE} with non-\code{NULL} \code{focal}.
#' 
#' @seealso
#' * [bal.tab()]
#' * [bal.tab.data.frame()]
