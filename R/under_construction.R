...
#Measures: correlation between variable and treatment; Distance correlation https://en.wikipedia.org/wiki/Distance_correlation
base.bal.tab.cont <- function(object, weights, treat, distance = NULL, subclass = NULL, covs, call = NULL, int = FALSE, addl = NULL, r.threshold = NULL, un = FALSE, method = "weighting", cluster = NULL, which.cluster = NULL, cluster.summary = TRUE) {
    ...
    balance.table.cont <- function(covs, weights, treat, distance, int, addl, continuous, binary, r.threshold = NULL, cluster = NULL) {
        #C=frame of variables, including distance; distance name (if any) stores in attr(C, "distance.name")
    ...
    #Actions
    if (length(cluster) > 0) {
        stop("Cluster are not currently supported with continuous treatments.")
        ...
        out <- vector("list", length(out.names))
        names(out) <- out.names
        out$Balance <- balance.table.cont(covs, weights, treat, distance, int, addl, continuous, binary, r.threshold, cluster)
        if (!is.null(r.threshold)) {
            r.threshcheck <- ifelse(is.null(weights), "Corr.Un", "Corr.Adj")
