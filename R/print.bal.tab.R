...
print.bal.tab.cluster <- function(x, disp.m.threshold = "as.is", disp.v.threshold = "as.is", disp.ks.threshold = "as.is", disp.r.threshold = "as.is", imbalanced.only = "as.is", un = "as.is", disp.bal.tab = "as.is", disp.means = "as.is", disp.sds = "as.is", disp.v.ratio = "as.is", disp.ks = "as.is", which.cluster, cluster.summary = "as.is", cluster.fun = "as.is", digits = max(3, getOption("digits") - 3), ...) {
    ...
    if (is_not_null(which.cluster)) {
        ...
        for (i in which.cluster) {
            ...
            if (all(c("Matched (ESS)", "Matched (Unweighted)") %in% rownames(c.balance[[i]][["Observations"]])) && 
                all(check_if_zero(c.balance[[i]][["Observations"]]["Matched (ESS)",] - c.balance[[i]][["Observations"]]["Matched (Unweighted)",]))) {
                c.balance[[i]][["Observations"]] <- c.balance[[i]][["Observations"]][rownames(c.balance[[i]][["Observations"]])!="Matched (Unweighted)", , drop = FALSE]
                rownames(c.balance[[i]][["Observations"]])[rownames(c.balance[[i]][["Observations"]]) == "Matched (ESS)"] <- "Matched"
            }
    ...
    if (isTRUE(as.logical(p.ops$imp.summary)) && is_not_null(i.balance.summary)) {
        ...
        if (is_not_null(which.cluster)) {
            ...
            for (c in which.cluster) {
                ...
                if (all(c("Matched (ESS)", "Matched (Unweighted)") %in% rownames(i.balance.c.summary[[c]][["Cluster.Observations"]])) && 
                    all(check_if_zero(i.balance.c.summary[[c]][["Cluster.Observations"]]["Matched (ESS)",] - i.balance.c.summary[[c]][["Cluster.Observations"]]["Matched (Unweighted)",]))) {
                    i.balance.c.summary[[c]][["Cluster.Observations"]] <- i.balance.c.summary[[c]][["Cluster.Observations"]][rownames(i.balance.c.summary[[c]][["Cluster.Observations"]])!="Matched (Unweighted)", , drop = FALSE]
                    rownames(i.balance.c.summary[[c]][["Cluster.Observations"]])[i.balance.c.summary[[c]][["Cluster.Observations"]] == "Matched (ESS)"] <- "Matched"
                }
        ...
        if (is_not_null(nn)) {
            ...
            if (all(c("Matched (ESS)", "Matched (Unweighted)") %in% rownames(x$Observations)) && 
                all(check_if_zero(x$Observations["Matched (ESS)",] - x$Observations["Matched (Unweighted)",]))) {
                x$Observations <- x$Observations[rownames(x$Observations)!="Matched (Unweighted)", , drop = FALSE]
                rownames(x$Observations)[x$Observations == "Matched (ESS)"] <- "Matched"
            }
            cat(underline(attr(x$Observations, "tag")) %+% "\n")
            print.warning <- FALSE
            if (length(attr(x$Observations, "ss.type")) > 1 && nunique.gt(attr(x$Observations, "ss.type")[-1], 1)) {
        ...
        if (is_not_null(nn)) {
            ...
            for (ti in seq_along(x$Observations)) {
                ...
                if (all(c("Matched (ESS)", "Matched (Unweighted)") %in% rownames(x$Observations[[ti]])) && 
                    ...
                    rownames(x$Observations[[ti]])[x$Observations[[ti]] == "Matched (ESS)"] <- "Matched"
                }
