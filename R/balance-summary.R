...
#'     \item{`col_w_ent()` computes the entropic distance for a set of covariates, a binary treatment, and weights (based on Oyenubi and Wittenerg, 2021).}
...
#' 
#' `col_w_ent()` computes the entropic distance as described by Oyenubi and Wittenberg (2021). The entropic distance is similar to the complement of the overlapping coefficient and is computed in the same way (i.e., using kernel density estimation and integration), except that the covariate densities in the treatment groups are contrasted in a different way. See Oyenubi and Wittenberg (2021) for details.
#' 
...
#' 
#' Oyenubi, A., & Wittenberg, M. (2021). Does the choice of balance-measure matter under genetic matching? *Empirical Economics*, 61(1), 489–502. \doi{10.1007/s00181-020-01873-9}
#' 
...
col_w_mean <- function(mat, weights = NULL, s.weights = NULL, subset = NULL, na.rm = TRUE, ...) {
    ...
    .chk_null_or(subset, chk::chk_logical)
...
col_w_ovl <- function(mat, treat, weights = NULL, s.weights = NULL, bin.vars, integrate = FALSE,
    ...
    if (any(!bin.vars)) {
        ...
        ovl[!bin.vars] <- apply(mat[, !bin.vars, drop = FALSE], 2, function(cov) {
            ...
            if (min(cov[treat == tval1]) > max(cov[treat != tval1]) ||
                min(cov[treat != tval1]) > max(cov[treat == tval1])) return(1)
...
#' @rdname balance-summary
#' @export 
col_w_ent <- function(mat, treat, weights = NULL, s.weights = NULL, bin.vars, integrate = FALSE,
                      subset = NULL, na.rm = TRUE, ...) {
    ...
    if (sum(weights[treat==tval1] > 0) < 1 || 
        sum(weights[treat!=tval1] > 0) < 1) {
        .err("at least 1 unit in each level of `treat` must have a nonzero weight to compute weighted entropic distance statistics")
    }
    ...
    t.sizes <- setNames(vapply(unique.treat, function(x) sum(treat == x), numeric(1L)),
                        unique.treat)
    smallest.t <- names(t.sizes)[which.min(t.sizes)]
    ent <- setNames(numeric(ncol(mat)), colnames(mat))
    if (any(!bin.vars)) {
        if (is_null(A[["bw"]])) A[["bw"]] <- "nrd"
        if (!is.function(get0(paste0("bw.", A[["bw"]])))) {
            .err(sprintf("%s is not an acceptable entry to `bw`. See `?stats::density` for allowable options",
        ...
        ent[!bin.vars] <- apply(mat[, !bin.vars, drop = FALSE], 2, function(cov) {
            if (na.rm) cov <- na.rem(cov)
            if (!na.rm && anyNA(cov)) return(NA_real_)
            if (min(cov[treat == tval1]) > max(cov[treat != tval1]) ||
                min(cov[treat != tval1]) > max(cov[treat == tval1])) return(1)
    ...
    if (any(bin.vars)) {
        ent[bin.vars] <- apply(mat[, bin.vars, drop = FALSE], 2, function(cov) {
          .5 * sum(vapply(unique(cov), function(i) {
              (sqrt(w.m(cov[treat == tval1] == i, weights[treat == tval1])) -
                  sqrt(w.m(cov[treat != tval1] == i, weights[treat != tval1])))^2
          }, numeric(1L)))
        })
    }
