#' @title Balance Statistics for \code{twang} Objects
#' 
#' @description
...
#' @param x a \code{ps}, \code{mnps}, \code{iptw}, or \code{ps.cont} object; the output of a call to \pkgfun{twang}{ps}, \pkgfun{twang}{mnps}, \pkgfun{twang}{iptw} or \pkgfun{twangContinuous}{ps.cont}.
...
#' 
#' @details
#' \code{bal.tab.ps()} generates a list of balance summaries for the input object given, and functions similarly to \pkgfun{twang}{bal.table}. The variances used in the denominator of the standardized mean differences computed in `twang::bal.table()` are weighted and computed using \code{survey::svyvar()} and are unweighted here (except when \code{s.weights} are specified, in which case \code{\link{col_w_sd}} is used). \pkg{twang} also uses "all" as the default \code{s.d.denom} when the estimand is the ATE; the default here is "pooled". For these reasons, results may differ slightly between the two packages. 
#' 
#' @seealso
