...
- x:
  ...
  an `ebalance` object (the output of a call to
  [`ebal::ebalance()`](https://rdrr.io/pkg/ebal/man/ebalance.html) or
  [`ebal::ebalance.trim()`](https://rdrr.io/pkg/ebal/man/ebalance.trim.html)
  ).
...
`bal.tab.ebalance()` must include either both `formula` and `data` or
both `covs` and `treat`.
