...
- x:
  ...
  an `sbwcau` object; the output of a call to
  [`sbw::sbw()`](https://rdrr.io/pkg/sbw/man/sbw.html) .
...
- s.d.denom:
  ...
  [`bal.tab()`](https://ngreifer.github.io/cobalt/reference/bal.tab.md)
  will figure out which one is best based on the `par` component of the
  `sbwcau` object: if "att", `"treated"`; if "atc", `"control"`;
  otherwise `"pooled"`.
...
If clusters are not specified, an object of class `"bal.tab"` containing
balance summaries for the `sbwcau` object. See
[`bal.tab()`](https://ngreifer.github.io/cobalt/reference/bal.tab.md)
for details.
...
`bal.tab.sbwcau()` generates a list of balance summaries for the
`sbwcau` object given, and functions similarly to
[`sbw::summarize()`](https://rdrr.io/pkg/sbw/man/summarize.html) .
