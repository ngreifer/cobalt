...
* `bal.tab()` now reports sample sizes for longitudinal treatments even when there is no balance summary across time points. Previously the `Observations` table was produced only alongside the summary, so a list of time points that could not be summarized together -- a mix of continuous and binary treatments, for instance -- reported no sample sizes at all.
