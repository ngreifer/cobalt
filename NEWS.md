...
* Fixed bugs that would occur when using `bal.tab()` without objects from the `Matching` package. Calculated statistics are now the same as those generated using `Matching::MatchBalance`. Changes based on updates to `get.w.Match()`.
