...
* Every heading and divider in a printed `bal.tab` object is now drawn the same way: the label centered in a rule, the form `WeightIt::summary.weightitMSM()` uses. This replaces the underlined headings and the ` - - - Label - - - ` dividers, and the closing rule that used to follow a set of them is gone, since each rule is already a full-width one. The rule's width responds to what is printed under it: every rule in one printout is drawn to the width of the widest line in it, which is normally the balance table, and never narrower than 44 characters or than the label itself. Only the presentation changed; the objects `bal.tab()` returns are untouched.
