...
We can use `which.time` to focus in on specific time points. `which.time` controls which time points are displayed; one or more time points can be entered. `.none`, the default, will not display any time points (so only the summary will be displayed), and `.all` will display all time points. Setting `msm.summary` to `FALSE` will hide the summary across time points. This option can be set globally using `set.cobalt.options(msm.summary = FALSE)`.

```{r}
bal.tab(Wmsm, which.time = 1:2)
```
