...
Greifer, N. (2016). cobalt: Covariate Balance Tables and Plots. R package version 1.2.0.
...
Users can create plots that display balance for a given cluster when the data are clustered. To do so, the `bal.tab` argument to `love.plot()` must contain an argument to `cluster` denoting cluster membership and an argument to `which.cluster` specifying for which single cluster balance is to be displayed in the plot. See the section "Using bal.tab with MatchIt" for details on how to use `bal.tab()` with clustered data.
