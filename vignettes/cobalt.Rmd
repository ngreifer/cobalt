...
bal.tab(m.out.subclass, disp.subclass = TRUE)
```
...
Generlaized boosted regression (GBM), as used in twang, can be an effective way to generate propensity scores and weights for use in propensity score weighting. `bal.tab()` functions similarly to the functions `bal.table()` and `summary()` when used with GBM in twang. Below is a simple example of its use:
