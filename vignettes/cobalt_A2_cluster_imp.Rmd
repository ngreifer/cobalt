...
Note that the `data` argument can be supplied as a `mids` objects, the output of a call to `mice()`. In this case, an argument o `imp` is unnecessary because the input is automaticallu extracted from the `mids` object. When this is done, though, the argument to `weights` cannot be given as names of variables in the data and must be supplied as a vector or data.frames.
