...
The next options only affect display, not the calculation of any statistics. First are `disp.means` and `disp.sds`, which controls whether the group means and standard deviations for each covariate are displayed in addition to the mean differences. This can be useful in reporting balance for publication, but in balance assessment mean differences are typically more useful.
...
See `?display_options` for the full list of display options. They can also be set as global options by using, for example, `options(cobalt_disp.means = TRUE)`, which allows user not to type a non-default options every time they call `bal.tab`.
