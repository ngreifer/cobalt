...
The next options only affect display, not the calculation of any statistics. First are `disp.means` and `disp.sds`, which control whether the group means and standard deviations for each covariate are displayed in addition to the mean differences. These can be useful in reporting balance for publication, but for balance assessment, mean differences are typically more useful.
