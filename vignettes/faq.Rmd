...
First, it is important to know that by default, **mean differences for binary covariates are not standardized**. That means in the `Diff.Adj`
...
-   `"all"`: $s^* = s$
-   `"weighted"`: $s^* = s_w$
-   `"hedges"`: $s^* = \frac{1}{1 - \frac{3}{4(n - 2) - 1}}\sqrt{\frac{(n_1 - 1)s_1^2 + (n_0 - 1)s_0^2}{n - 2}}$
...
Use `as.data.frame()` to get the balance statistics as data, or
`format()` to get the table exactly as `print()` displays it.
...
Set `wide = TRUE` to get the layout `print()` uses instead, with one
...
`format()` returns the printed table as a data frame of formatted
strings---rounded, decimal-aligned, with missing values shown as
`.`---which is what you want when the destination is a document rather
than more code:
...
Both functions accept every argument `print()` accepts---`stats`,
`disp`, `un`, `imbalanced.only`, `disp.thresholds`, the `which.*`
arguments---and resolve them the same way, so you can select what to
report without recomputing anything:
...
want to report them under. Give it to `bal.tab()` and it applies
wherever the object is displayed afterwards, including `love.plot()`;
...
displayed names change---`b$Balance` still has the stored ones---so
nothing downstream has to know about them. See `?display-options` for
the other ways to specify it.
