...
```
The `sbw` package implements optimization-based weighting to estimate weights that satisfy certain balance constraints and have minimal variance. `bal.tab()` functions similarly to `sbw`'s `summarize()` command but provides additional flexibility and convenience. Below is a simple example of using `bal.tab()` with `sbw`:
```{r}
library("sbw")
data("lalonde", package = "cobalt") #If not yet loaded
