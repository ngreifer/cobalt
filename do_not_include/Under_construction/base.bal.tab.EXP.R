...
base.bal.tab.binary <- function(weights, treat, distance = NULL, subclass = NULL, covs, call = NULL, int = FALSE, poly = 1, addl = NULL, continuous = getOption("cobalt_continuous", "std"), binary = getOption("cobalt_binary", "raw"), s.d.denom, m.threshold = NULL, v.threshold = NULL, ks.threshold = NULL, imbalanced.only = getOption("cobalt_imbalanced.only", FALSE), un = getOption("cobalt_un", FALSE), disp.means = getOption("cobalt_disp.means", FALSE), disp.sds = getOption("cobalt_disp.sds", FALSE), disp.v.ratio = getOption("cobalt_disp.v.ratio", FALSE), disp.ks = getOption("cobalt_disp.ks", FALSE), disp.subclass = getOption("cobalt_disp.subclass", FALSE), disp.bal.tab = getOption("cobalt_disp.bal.tab", TRUE), method, imp = NULL, which.imp = NA, imp.summary = getOption("cobalt_imp.summary", TRUE), imp.fun = getOption("cobalt_imp.fun", NULL), s.weights = NULL, discarded = NULL, abs = FALSE, quick = TRUE, addl.sds = NULL, ...) {
    ...
    if (is_not_null(args[["agg.fun"]])) imp.fun <- args[["agg.fun"]]
    
    #Actions
    if (is_null(imp)) {
        if (length(method) == 1 && method == "subclassification") {
            if (is_null(subclass)) stop("Method specified as subclassification, but no subclasses were specified.", call. = FALSE)
        ...
        args_to_NULL <- c("m.threshold", "v.threshold", "ks.threshold", "imp")
        null_args <- setNames(vector("list", length(args_to_NULL)), args_to_NULL)
        args_not_subsetted <- ls()[ls() %nin% c("weights", "treat", "distance",
                                                "subclass", "covs", "addl", 
                                                "s.weights",
                                                "discarded",
                                                args_to_NULL)]
        base_args <- mget(args_not_subsetted[args_not_subsetted %in% names(formals(base.bal.tab.binary))])
...
base.bal.tab.cont <- function(weights, treat, distance = NULL, subclass = NULL, covs, call = NULL, int = FALSE, poly = 1, addl = NULL, r.threshold = NULL, imbalanced.only = getOption("cobalt_imbalanced.only", FALSE), un = getOption("cobalt_un", FALSE), disp.means = getOption("cobalt_disp.means", FALSE), disp.sds = getOption("cobalt_disp.sds", FALSE), disp.subclass = getOption("cobalt_disp.subclass", FALSE), disp.bal.tab = getOption("cobalt_disp.bal.tab", TRUE), method, imp = NULL, which.imp = NA, imp.summary = getOption("cobalt_imp.summary", TRUE), imp.fun = getOption("cobalt_imp.fun", NULL), s.weights = NULL, discarded = NULL, abs = FALSE, quick = TRUE, ...) {
    ...
    if (is_null(imp))  {
        ...
        args_to_NULL <- c("r.threshold", "imp")
        null_args <- setNames(vector("list", length(args_to_NULL)), args_to_NULL)
        args_not_subsetted <- ls()[ls() %nin% c("weights", "treat", "distance",
                                                "subclass", "covs", "addl", 
                                                "s.weights",
                                                "discarded",
                                                args_to_NULL)]
        base_args <- mget(args_not_subsetted[args_not_subsetted %in% names(formals(base.bal.tab.cont))])
...
base.bal.tab.multi <- function(weights, treat, distance = NULL, subclass = NULL, covs, call = NULL, int = FALSE, poly = 1, addl = NULL, continuous = getOption("cobalt_continuous", "std"), binary = getOption("cobalt_binary", "raw"), s.d.denom, m.threshold = NULL, v.threshold = NULL, ks.threshold = NULL, imbalanced.only = getOption("cobalt_imbalanced.only", FALSE), un = getOption("cobalt_un", FALSE), disp.means = getOption("cobalt_disp.means", FALSE), disp.sds = getOption("cobalt_disp.sds", FALSE), disp.v.ratio = getOption("cobalt_disp.v.ratio", FALSE), disp.ks = getOption("cobalt_disp.ks", FALSE), disp.subclass = getOption("cobalt_disp.subclass", FALSE), disp.bal.tab = getOption("cobalt_disp.bal.tab", TRUE), method, imp = NULL, which.imp = NA, imp.summary = getOption("cobalt_imp.summary", TRUE), imp.fun = getOption("cobalt_imp.fun", NULL), pairwise = TRUE, focal = NULL, which.treat = NA, multi.summary = getOption("cobalt_multi.summary", TRUE), s.weights = NULL, discarded = NULL, abs = FALSE, quick = TRUE, ...) {
    #Preparations
    args <- list(...)
    if (is_not_null(m.threshold)) m.threshold <- abs(m.threshold)
    if (is_not_null(v.threshold)) {
    ...
    if (is_not_null(args[["agg.fun"]])) imp.fun <- args[["agg.fun"]]
    if (is_null(imp)) imp <- NULL #otherwise factor of length 0
    ...
    if (is_null(focal)) {
        ...
        if (pairwise || is_not_null(focal)) {
            args <- args[names(args) %nin% names(formals(base.bal.tab.binary))]
            args_not_set <- ls()[ls() %nin% c("weights", "treat", "distance", "subclass", "covs", "call", "addl",
                                              "imp", "s.weights", "discarded")]
            base_args <- mget(args_not_set[args_not_set %in% names(formals(base.bal.tab.binary))])
            balance.tables <- lapply(treat.combinations, function(t) {
                do.call(base.bal.tab.binary, 
                        c(list(weights = weights[treat %in% t, , drop = FALSE],
                               treat = factor(treat[treat %in% t], t),
                               distance = distance[treat %in% t, , drop = FALSE], 
                               subclass = subclass[treat %in% t], 
                               covs = covs[treat %in% t, , drop = FALSE], 
                               call = NULL, 
                               addl = addl[treat %in% t, , drop = FALSE],
                               imp = imp[treat %in% t],
                               s.weights = s.weights[treat %in% t],
                               discarded = discarded[treat %in% t]
                        ), 
                        base_args,
                        args), quote = TRUE)
            })
            ...
            args <- args[names(args) %nin% names(formals(base.bal.tab.binary))]
            args_not_set <- ls()[ls() %nin% c("treat", "call")]
            base_args <- mget(args_not_set[args_not_set %in% names(formals(base.bal.tab.binary))])
            balance.tables <- lapply(treat.combinations, function(t) {
        ...
        out[["Observations"]] <- samplesize.multi(balance.tables, treat_names(treat), focal)
...
base.bal.tab.msm <- function(weights, treat.list, distance.list = NULL, subclass = NULL, covs.list, call = NULL, int = FALSE, poly = 1, addl.list = NULL, continuous = getOption("cobalt_continuous", "std"), binary = getOption("cobalt_binary", "raw"), s.d.denom, m.threshold = NULL, v.threshold = NULL, ks.threshold = NULL, r.threshold = NULL, imbalanced.only = getOption("cobalt_imbalanced.only", FALSE), un = getOption("cobalt_un", FALSE), disp.means = getOption("cobalt_disp.means", FALSE), disp.sds = getOption("cobalt_disp.sds", FALSE), disp.v.ratio = getOption("cobalt_disp.v.ratio", FALSE), disp.ks = getOption("cobalt_disp.ks", FALSE), disp.bal.tab = getOption("cobalt_disp.bal.tab", TRUE), method, cluster = NULL, which.cluster = NULL, cluster.summary = getOption("cobalt_cluster.summary", TRUE), cluster.fun = getOption("cobalt_cluster.fun", NULL), pairwise = TRUE, focal = NULL, which.treat = NA, multi.summary = getOption("cobalt_multi.summary", TRUE), which.time = NULL, msm.summary = getOption("cobalt_msm.summary", TRUE), imp = NULL, which.imp = NA, imp.summary = getOption("cobalt_imp.summary", TRUE), imp.fun = getOption("cobalt_imp.fun", NULL), s.weights = NULL, discarded = NULL, abs = FALSE, quick = TRUE, ...) {
    ...
    if (nunique.gt(cluster, 1)) {
        ...
        stop("Subclassification not yet supported with longitudinal treatments.", call. = FALSE)
    }
    else{
        #Setup output object
        ...
        out[["Time.Balance"]] <- vector("list", length(treat.list))
        for (ti in seq_along(treat.list)) {
            if (is_not_null(imp)) {
            ...
            else if (treat.type[ti] == "continuous") {
                args <- args[names(args) %nin% names(formals(base.bal.tab.cont))]
                args_not_subsetted <- ls()[ls() %nin% c("treat", "distance",
                                                        "covs", "addl", "call")]
                base_args <- mget(args_not_subsetted[args_not_subsetted %in% names(formals(base.bal.tab.cont))])
                out[["Time.Balance"]][[ti]] <- do.call(base.bal.tab.cont, 
                                                       c(list(treat = treat.list[[ti]], 
                                                              distance = distance.list[[ti]], 
                                                              covs = covs.list[[ti]], 
                                                              addl = addl.list[[ti]],
                ...
                args_not_subsetted <- ls()[ls() %nin% c("treat", "distance",
                                                        "covs", "addl", "call")]
                base_args <- mget(args_not_subsetted[args_not_subsetted %in% names(formals(base.bal.tab.multi))])
                out[["Time.Balance"]][[ti]] <- do.call(base.bal.tab.multi, 
                                                       c(list(treat = treat.list[[ti]], 
                                                              distance = distance.list[[ti]], 
                                                              covs = covs.list[[ti]], 
                                                              addl = addl.list[[ti]],
                ...
                args_not_subsetted <- ls()[ls() %nin% c("treat", "distance",
                                                        "covs", "addl", "call")]
                base_args <- mget(args_not_subsetted[args_not_subsetted %in% names(formals(base.bal.tab.binary))])
                out[["Time.Balance"]][[ti]] <- do.call(base.bal.tab.binary, 
                                                       c(list(treat = treat.list[[ti]], 
                                                              distance = distance.list[[ti]], 
                                                              covs = covs.list[[ti]], 
                                                              addl = addl.list[[ti]],
        ...
        if (!(quick && !msm.summary) && all_the_same(treat.type) && !any(treat.type == "multinomial") && is_null(imp)) {
